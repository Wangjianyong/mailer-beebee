...
class WorkerThread(threading.Thread):
    ...
    def __init__(self, initrequests_queue, requests_queue, results_queue, poll_timeout=5, **kwds):
        ...
        self._initflag = True
        self._initrequests_queue = initrequests_queue
    ...
    def run(self):
        """some initial work needs to be done before doing the real job
           then Repeatedly process the job queue until told to exit."""
        while True:
            ...
            if self._initflag == True:
                try:
                    initrequest = self._initrequests_queue.get(True, self._poll_timeout)
                except Queue.Empty:
                    #if there is no producer any more, just stop the thread.
                    if not self._initreqproducer.isAlive():
                        break
                    continue
                else:
                    if self._dismissed.isSet():
                        break
                    res = initrequest.callable(self.threadID, initrequest.args, initrequest.kwds)
            ...
            try:
                ...
                if not self._sendreqproducer.isAlive():
                    break
                continue
            else:
                if self._dismissed.isSet():
                    ...
                    break
                try:
                    request.kwds.update(res)
                    result = request.callable(self.threadID, request.args, request.kwds)
                    if result['Status'] is False:
                        # error happens in current thread, redo initial work & put back request in queue
                        self._initflag = True
                        self._requests_queue.put(request)
                        # in some cases, session will be terminated by smtp server, we should put the
                        # init request back to the initqueue
                        if result['Retcode'] == 502:
                            self._initrequests_queue.put(initrequest)
                        continue
    ...
    def setproducer(self, initreqproducer, sendreqproducer):
        self._initreqproducer = initreqproducer
        self._sendreqproducer = sendreqproducer


class InitRequest:
    """A init job needs to be done before starting the real job"""
    def __init__(self, callable_, args=None, kwds=None, initrequestID=None):
        if initrequestID is None:
            self.initrequestID = id(self)
        else:
            try:
                self.initrequestID = hash(initrequestID)
            except TypeError:
                raise TypeError("initrequestID must be hashable.")
...
class ThreadPool:
    ...
    def __init__(self, num_workers, dbhandle=None, i_size=0, q_size=0, resq_size=0, poll_timeout=5):
        ...
        """
        self._initrequests_queue = Queue.Queue(i_size)
    ...
    def createWorkers(self, num_workers, poll_timeout=5):
        ...
        for i in range(num_workers):
            self.workers.append(WorkerThread(self._initrequests_queue, 
    ...
    def putInitRequest(self, initrequest, block=True, timeout=None):
        assert isinstance(initrequest, InitRequest)
        self._initrequests_queue.put(initrequest, block, timeout)
    ...
    def workersloopcheck(self):
        """check all threads in pool & put dead workers into dismissedWorkers"""
        for worker in self.workers:
            if not worker.isAlive():
                self.workers.remove(worker)
                self.dismissedWorkers.append(worker)

    def poll(self, block=False):
        """Process any new results in the queue."""
        while True:
            self.workersloopcheck()
            # still results pending?
            if not self.workRequests:
                raise NoResultsPending
            # are there still workers to process remaining requests?
            elif block and not self.workers and not self._results_queue.qsize():
                raise NoWorkersAvailable
            try:
                # get back next results
                request, result = self._results_queue.get(block=False)
                ...
                if request.callback and not \
                    ...
                    #put dbhandle into result dic
                    result["dbhandle"] = self._dbhandle
    ...
    def setproducer(self, initreqproducer, sendreqproducer):
       """set init producer and send producer for each thread"""
       for worker in self.workers:
            worker.setproducer(initreqproducer, sendreqproducer)
